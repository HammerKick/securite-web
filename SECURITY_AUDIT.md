...
Le projet reste assez simple et dans tous les cas, l'admin possède toutes les infos donc si on arrive à accéder à l'user admin via un bruteforce ou une interception HTTP et qu'on arrive à déchiffrer le mot de passe, on aura accès à tout.
En dehors de ça, le reste est sécurisé, attention cependant à ce qu'un spam de calls API en ajoutant des commandes ne bourre pas trop l'api.
